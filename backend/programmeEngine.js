...
const CATALOGUE_OCTOGO = [
  { code: 'EVEIL', titre: 'ÉVEIL — NeuroLeadership & Performance Collective', categorie: 'LEADERSHIP', dureeJoursParDefaut: 3,
  ...
  { code: 'RESET', titre: 'RESET — Neuromanagement & Performance durable', categorie: 'MANAGEMENT', dureeJoursParDefaut: 3,
  ...
  { code: 'CAPTE', titre: 'CAPTE — Le cerveau au cœur de la relation client', categorie: 'RELATION CLIENT', dureeJoursParDefaut: 2,
    motsCles: ['client', 'relation client', 'service client', 'satisfaction', 'fidélisation', 'accueil', 'support', 'expérience client'] },
  { code: 'NEUROVENTE', titre: 'NEUROVENTE — Vendre au cerveau pour toucher le cœur', categorie: 'VENTE', dureeJoursParDefaut: 2,
    motsCles: ['vente', 'commercial', 'négociation', 'persuasion', 'influence', 'convaincre', 'argumentation', 'vendre', 'closing', 'prospection'] },
  { code: 'NEUROMARKETING', titre: 'NEUROMARKETING — Le cerveau au service de la stratégie', categorie: 'MARKETING', dureeJoursParDefaut: 2,
    motsCles: ['marketing', 'communication', 'publicité', 'marque', 'consommateur', 'stratégie marketing', 'digital', 'contenu'] },
  { code: 'TRANSFORMATION', titre: 'TRANS-FORMATION — Leadership transformationnel & intelligence émotionnelle', categorie: 'TRANSFORMATION', dureeJoursParDefaut: 3,
  ...
  { code: 'VOLDAIGLE', titre: 'VOL D\u2019AIGLE — Vision stratégique & lecture de l\u2019environnement', categorie: 'STRATÉGIE', dureeJoursParDefaut: 2,
...
function apparierProgramme(texte) {
  ...
  for (const prog of CATALOGUE_OCTOGO) {
    let score = 0;
    for (const mot of prog.motsCles) if (base.includes(normaliser(mot))) score += 1;
    if (score > meilleurScore) { meilleurScore = score; meilleur = prog; }
  }
...
function formateurDisponiblePourMois(formateur, annee, mois) {
  ...
  for (const bloc of indispos) {
    const d = new Date(bloc.debut), f = new Date(bloc.fin || bloc.debut);
...
function genererProgrammeAnnuel(state, formateurs) {
  const s = state || {};
  ...
  for (const metier of metiersEffectifs) {
    ...
    for (const { prog, objectifs: objs } of programmesParMetier.values()) {
      const dureeJours = prog.dureeJoursParDefaut;
      const mois = ((curseurMois - 1) % 12) + 1;
      const coutEstime = dureeJours * nbGroupes * tarifJour;
      actions.push({
        id: uid('act'), intituleAction: prog.titre, programmeSource: prog.code, categorie: prog.categorie,
        corpsMetier: metier.intitule, effectif, nbGroupes, objectifLie: [...objs].join(' · '),
        competences: prog.competencesDeveloppees || [], dureeJours, mois, moisLibelle: MOIS_FR[mois - 1],
        trimestre: trimestreDepuisMois(mois),
        modalite: effectif > TAILLE_GROUPE_PAR_DEFAUT ? 'Présentiel multi-groupes' : 'Présentiel / Hybride',
        coutEstime, coutLibelle: `${coutEstime.toLocaleString('fr-FR')} ${devise} HT`,
      });
      curseurMois += 1;
    }
  }
  ...
  const planParMetier = metiersEffectifs.map((metier) => {
    ...
    return { corpsMetier: metier.intitule, effectif: Math.max(0, toNombre(metier.effectif, 0)),
      priorite: metier.priorite || 'Normale', missions: metier.missions || '', actions: am,
      joursTotal: am.reduce((t, a) => t + a.dureeJours * a.nbGroupes, 0), coutTotal: am.reduce((t, a) => t + a.coutEstime, 0) };
  });
  ...
  const budget = { devise, tarifJour, coutTotalEstime, coutTotalLibelle: `${coutTotalEstime.toLocaleString('fr-FR')} ${devise} HT`,
    budgetDisponible, ecart, statut: statutBudget,
    repartitionParMetier: planParMetier.map((p) => ({ corpsMetier: p.corpsMetier, cout: p.coutTotal })), repartitionParTrimestre };
  ...
  const meta = {
    entreprise: ent.nom || 'Entreprise', domaineActivite: ent.domaineActivite || '', annee,
    moisDemarrage: MOIS_FR[moisDepart - 1], caracteristiques: ent.caracteristiques || '',
    strategieGlobale: ent.strategieGlobale || '', strategieRH: ent.strategieRH || '',
    dossierTFP: !!(s.cadreTFP && s.cadreTFP.dossierTFP), detailsTFP: (s.cadreTFP && s.cadreTFP.detailsTFP) || '',
    dateGeneration: new Date().toLocaleDateString('fr-FR', { day: '2-digit', month: 'long', year: 'numeric' }),
    nbActions: actions.length, joursTotal: actions.reduce((t, a) => t + a.dureeJours * a.nbGroupes, 0),
